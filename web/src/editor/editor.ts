...
export class Editor extends EventTarget {
  ...
  private createElement(
    ...
    parent.innerHTML = `<div class="code-editor">
                <div class="gutter"><reading>1</reading></div>
                <div class="input" contenteditable="true" autocapitalize="off" autocomplete="off" spellcheck="false" autocorrect="off"></div>
            </div>`;

    const gutter = parent.querySelector<HTMLDivElement>(
      "div.code-editor div.gutter",
    )!;
    const input = parent.querySelector<HTMLDivElement>(
      "div.code-editor div.input",
    )!;
  ...
  private render() {
    ...
    lines.forEach(({ fragments }, lineIndex) => {
      if (fragments.length === 0) {
        ...
        gutter_html += `<reading>${lineIndex + 1}</reading>`;
      ...
      fragments.forEach(({ value, kind, error, hint }) => {
        if (kind || error || hint) {
          ...
          input_html += `<reading class="${classes}" title="${title}">${value}</reading>`;
        } else {
          input_html += `<reading>${value}</reading>`;
        }
      ...
      if (contains_error) {
        gutter_html += `<reading class="error">${lineIndex + 1}</reading>`;
      } else if (contains_move) {
        gutter_html += `<reading class="move">${lineIndex + 1}</reading>`;
      } else {
        gutter_html += `<reading>${lineIndex + 1}</reading>`;
      }
    });
  ...
  getCaret(): Caret | null {
    ...
    try {
      ...
      if (
        ...
        return {
          from: {
            lineIndex: this.getCaretLineIndex(
              range.startContainer,
              range.startOffset,
            ),
            charIndex: this.getCaretCharIndex(
              range.startContainer,
              range.startOffset,
            ),
          },
  ...
  insertText(text: string): Caret {
    ...
    return {
      from: {
        charIndex:
          data.length === 1
            ? caret.from.charIndex + data[0].length
            : data[data.length - 1].length,
        lineIndex:
          data.length === 1
            ? caret.from.lineIndex
            : caret.from.lineIndex + data.length - 1,
      },
    };
  }
  ...
  deleteBackward(word?: boolean): Caret {
    ...
    if (caret.from.charIndex === 0 && caret.from.lineIndex === 0) {
      return caret;
    }
    ...
    if (caret.from.charIndex === 0) {
      ...
      this.content = mergeLines(
        this.content,
        caret.from.lineIndex - 1,
        caret.from.lineIndex,
      );
    ...
    this.content = removeChars(
      this.content,
      caret.from.lineIndex,
      caret.from.charIndex - 1,
      caret.from.charIndex,
    );
  ...
  deleteForward(word?: boolean): Caret {
    ...
    if (
      caret.from.charIndex === this.content[caret.from.lineIndex].length &&
